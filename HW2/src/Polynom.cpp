...
Polynom::Polynom(initializer_list<float> coef_list,
	...
	for (auto iteration : coef_list) {
	}

	for (auto iter = coef_list.begin(); iter != coef_list.end(); ++iter) {
		num_terms++;
		coeff_expon.push_back(std::make_pair((float) 2, 1)); // Inserting into vector
		// coeff_expon.push_back(std::make_pair((float) 2, 1));
		cout << "Coefficient: " << *iter << endl;
	}

	for (int expo : expo_list) {
